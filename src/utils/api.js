const API_BASE_URL = 'https://greenthumb-backend-dwst.onrender.com';
...
if (!response.ok) {
    throw new Error(`Failed to add plant: ${data.error}`);
}
...
// Fetch care schedules for a specific plant
export const fetchCareSchedules = async (plantId) => {
    const url = plantId
        ? `${API_BASE_URL}/care_schedules?plant_id=${plantId}`
        : `${API_BASE_URL}/care_schedules`;
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Failed to fetch care schedules');
        }
...
export const addCareSchedule = async (schedule) => {
    try {
...
export const updateCareSchedule = async (scheduleId, careScheduleData) => {
    try {
        ...
        if (!response.ok) {
            const data = await response.json();
            throw new Error(data.error || 'Failed to update care schedule');
        }
...
export const deleteCareSchedule = async (scheduleId) => {
    try {
        const response = await fetch(`${API_BASE_URL}/care_schedules/${scheduleId}`, {
            method: 'DELETE',
            headers: {
        ...
        if (!response.ok) {
            const data = await response.json();
            throw new Error(data.error || 'Failed to delete care schedule');
        }
